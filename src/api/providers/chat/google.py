...
class Gemini:
    ...
    @classmethod
    async def _get_client(cls, body: dict) -> tuple[str, GenerativeModel]:
        """Retrieve a valid Gemini client"""
    ...
    async def chat_completion(cls, body: dict) -> tuple[Union[StreamingResponse, dict], str]:
        ...
        key, model = await cls._get_client(body)
        
        if any(isinstance(value, list) for message in body["messages"] for value in message.values()):
